...
import type { HDAttachment } from '@/lib/types';
...
export default function AttachmentUploader({ helpDeskId, attachments, onUpdate }: AttachmentUploaderProps) {
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setUploading(true);
    try {
      await api.uploadAttachment(helpDeskId, file, file.name);
      onUpdate();
    } catch {
  ...
  async function handleUrlSubmit(e: React.FormEvent) {
    ...
    setUploading(true);
    try {
      await api.addUrlAttachment(helpDeskId, urlName.trim(), urlValue.trim());
      setUrlName('');
      setUrlValue('');
      onUpdate();
    } catch {
  ...
  async function handleDelete(attachmentId: number) {
    ...
    try {
      await api.deleteAttachment(helpDeskId, attachmentId);
      onUpdate();
    } catch {
